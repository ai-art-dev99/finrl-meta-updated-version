...
class AlpacaPaperTrading_rllib:
    def __init__(
        ...
        if agent == "ppo":
            ...
            # 1) Register your env (RLlib expects a *creator* that returns an Env instance)
            #    Your StockEnvEmpty should accept a single `env_config` dict.
            register_env("StockEnvEmpty", lambda env_config: StockEnvEmpty(env_config))

            # 2) Build the Algorithm from a typed config
            config = (
                PPOConfig()
                .environment(
                    env="StockEnvEmpty",
                ...
                # .framework("torch")  # uncomment if you need to force torch
            )
            algo = config.build()

            # 3) Restore (prefer new API; fall back to older .restore if needed)
            try:
                # Use this if the checkpoint was created via `algo.save_to_path()`
                algo.restore_from_path(cwd)
                print("Restoring (new API) from checkpoint dir:", cwd)
            except Exception:
                # Works if the checkpoint was created via older `algo.save()`/Tune
                algo.restore(cwd)
                print("Restoring (legacy API) from checkpoint path:", cwd)
