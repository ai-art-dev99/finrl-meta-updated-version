    ...
    def __init__(
        ...
        if agent == "ppo":
            if drl_lib == "elegantrl":
                ...
                # Register an env-creator (RLlib expects a callable)
                register_env("StockEnvEmpty", lambda cfg: StockEnvEmpty(cfg))

                # Build PPO via the new typed config API
                config = (
                    PPOConfig()
                    .environment(
                        env="StockEnvEmpty",
                ...
                # Restore (prefer new API; fall back if your checkpoint is from older Tune/Trainer)
                try:
                    # Use this if the checkpoint dir was created via algo.save_to_path()
                    algo.restore_from_path(cwd)
                    print("Restoring (new API) from checkpoint dir:", cwd)
                except Exception:
                    # Works for older checkpoints saved via algo.restore()/Tune
                    algo.restore(cwd)
